...
/**
 * Drops keys that collide with SDK-owned namespaces, page-state ownership,
 * or have non-primitive values. Returns a new object — never mutates input.
 */
const sanitizeMetadata = (
    input: EventMetadata | undefined,
    layer: 'application' | 'event' | 'hook'
): EventMetadata => {
    if (!input) {
        return {};
    }
    ...
    for (const [k, v] of Object.entries(input)) {
        if (k.startsWith('aws:')) {
            InternalLogger.warn(
                `Dropping ${layer} metadata key "${k}": "aws:" namespace is reserved.`
            );
            continue;
        }
        if (
            (layer === 'event' || layer === 'hook') &&
            RESERVED_PAGE_KEYS.has(k)
        ) {
            InternalLogger.warn(
                `Dropping ${layer} metadata key "${k}": owned by page state.`
            );
            continue;
        }
        const t = typeof v;
        if (t !== 'string' && t !== 'number' && t !== 'boolean') {
            InternalLogger.warn(
                `Dropping ${layer} metadata key "${k}": value must be string|number|boolean.`
            );
            continue;
        }
        out[k] = v;
...
export class EventCache {
    ...
    private readonly applicationAttributes: Readonly<EventMetadata>;
    private eventMetadataHook?: EventMetadataHook;
    ...
    constructor(
        ...
        this.applicationAttributes = Object.freeze(
            sanitizeMetadata(config.applicationAttributes, 'application')
        );
    }
    ...
    public recordEvent: RecordEvent = (
        ...
        if (this.isCurrentUrlAllowed()) {
            ...
            if (this.sessionManager.canRecord()) {
                ...
                this.addRecordToCache(type, eventData, metadata);
            } else {
    ...
    private addRecordToCache = (
        ...
        const [event, parsedEvent] = this.createEvent(
            type,
            eventData,
            metadata
        );
    ...
    /**
     * Register a hook that decorates every recorded event's metadata. The
     * hook receives the in-progress merged metadata (page attributes only —
     * manual metadata is applied after the hook) and returns additional
     * keys. Manual metadata always wins over hook output. Replaces any
     * previously set hook.
     */
    public setEventMetadataHook(hook: EventMetadataHook): void {
        this.eventMetadataHook = hook;
    }

    /** Remove a previously registered event metadata hook. */
    public clearEventMetadataHook(): void {
        this.eventMetadataHook = undefined;
    }
    ...
    private createEvent = (
        ...
        // contains only per-event attributes (page + hook + manual). Consumers
        // merge: { ...request.SessionMetadata, ...event.metadata } with event
        // keys winning on collision.
        const pageAttrs = this.pageManager.getAttributes() ?? {};
        const sanitizedManual = sanitizeMetadata(metadata, 'event');
        ...
        // Merge order: page (lowest) → hook → manual (highest).
        const eventMetadata = {
            ...pageAttrs,
            ...hookOutput,
            ...sanitizedManual
