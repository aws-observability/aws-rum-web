...
| spaActivityTimeoutLimit | Number | `1000` | The maximum timeout value (ms) for a virtual page load. If your application's virtual page does not load within the timeout value, the web client will not record the page load data. If your application takes longer time to load (i.e more than 10 seconds) and you wish to see the page load data, it is recommended to increase the timeout threshold.|
