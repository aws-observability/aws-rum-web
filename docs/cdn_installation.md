...
| enableSpaTracking | Boolean | `false` | When this field is `true`, the web client will initialize TrackerPlugin to capture virtual page load timing for single page applications. It is recommended to use the `pageIdFormat` as `PATH_AND_HASH` to allow the web client to identify route changes for virtual pages for automatic tracking.
...
| spaTimeoutLimit | Number | `1000` | The maximum timeout value (ms) for a virtual page load. If your application's virtual page does not load within the timeout value, the web client will not record the page load data. If your application takes longer time to load (i.e more than 10 seconds) and you wish to see the page load data, it is recommended to increase the timeout threshold.|
