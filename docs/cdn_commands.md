...
In the following example, the snippet has disabled automated page view recording and has not provided AWS credentials. Following the snippet, the application manually records a page view and forwards AWS credentials to the web client.
...
| recordEvent | [Event](#event), options: [RecordEventOptions](#recordeventoptions) | `cwr('recordEvent', {type: 'your_event_type', data: {field1: 1, field2: 2}})` <br/><br/> `awsRum.recordEvent('your_event_type', {field1: 1, field2: 2})`<br>`cwr('recordEvent', {type: 'runnin_max_type': data: 1}, {isCandidate: true, replaceFirstMatch: true})`<br>`cwr('recordEvent', {type: 'runnin_max_type': data: 2}, {isCandidate: true, replaceFirstMatch: true})`<br>`cwr('recordEvent', {type: 'runnin_max_type': data: 3}, {isCandidate: true, replaceFirstMatch: true})` | Record a custom event.<br/><br/>⚠️ The app monitor must have custom events enabled.<br/><br/> Event types must conform to the following regex: `^[a-zA-Z0-9_.-]{1,256}$`. <br/><br/>Each custom event, including metadata, must be less than 6 KB. Events over 6KB are dropped by CloudWatch RUM. <br/><br/> To record an event candidate that should not be sent to PutRumEvents until the end of the page lifecycle, then use the isCandidate=true option. To replace an event in the current cache, then use the replaceFirstMatch=true option. |
...
The RUM web client also records a set of [default attributes](https://github.com/aws-observability/aws-rum-web/blob/main/src/event-schemas/meta-data.json). You cannot overwrite default attributes with custom attributes.
...
| Field Name | Type | Default | Example | Description |
| --- | --- | --- | --- | --- |
| isCandidate | Boolean | FALSE | In this example, RUM will cache all 'running_max' candidates, but only call the final value=15 to PutRumEvents at the end of the Page's lifecycle. This is a very frugal solution if you need to keep a running count, but only want to publish the event at the latest possible time. <br/><br/>aws.recordEvent('final_click_count', {value: 5}, {isCandidate: true, replaceFirstMatch: true})<br/>aws.recordEvent('final_click_count', {value: 10}, {isCandidate: true, replaceFirstMatch: true})<br/>aws.recordEvent('final_click_count', {value: 15}, {isCandidate: true, replaceFirstMatch: true}) | If true, then RUM will not dispatch the candidate event to PutRumEvents until the next visibilitychange event, which only occurs when the page is hidden or closed.<br/><br/>Event candidates are prioritized over regular events in that final batched call because that is the only opportunity for them to be reported. <br/><br/>To provision more cache space for candidates, then increase candidatesCacheSize. |
| replaceFirstMatch | Boolean | FALSE | In this example, RUM will cache all 'running_click_count' events, but only a last event will be reported in the next batch. If the previous matching event types were already send to PutRumEvents, then additional events will be recorded. This is a good way to minimize the risk of dropping events while still reducing some redundant data. <br/><br/>aws.recordEvent('running_click_count', {value: 5}, {replaceFirstMatch: true})<br/>aws.recordEvent('running_click_count', {value: 10}, {replaceFirstMatch: true})<br/>aws.recordEvent('running_click_count', {value: 15}, {replaceFirstMatch: true}) | If true, then RUM will replace the first matching event type in the event cache. If isCandidate=true as well, then the first matching candidate type is replaced in the candidate cache. |
