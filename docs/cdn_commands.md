...
| pageAttributes | `{ [key: string]: string \| number \| boolean }` | N/A | `{ 'pageOrder': 1`, `'template': 'studio' }`  | A page attribute added to the metadata of all events with the provided `pageId`. Attribute keys must conform to the following regex: `^(?!pageTags)(?!aws:)[a-zA-Z0-9_:]{1,128}$`. Attribute values can have up to 256 characters and must be of type `string`, `number`, or `boolean`.|