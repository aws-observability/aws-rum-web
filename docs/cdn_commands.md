...
| recordEvent | Object | `cwr('recordEvent', {type: 'your_event_type', data: {field1: 1, field2: 2}})` <br/><br/> `awsRum.recordEvent('your_event_type', {field1: 1, field2: 2})` | Record a custom event. Event types must conform to the following regex: `^[a-zA-Z0-9_.-]{1,256}$`.<br/><br/> Event data must be an object that consists of fields and values. Each custom event can be up to 6 KB. Custom events will be searchable in RUM console's `Events` tab using the event type.
