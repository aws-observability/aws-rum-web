...
| allowDynamicDomEventListeners | Boolean | `cwr('allowDynamicDomEventListeners', true);`<br/><br/>`awsRum.allowDynamicDomEventListeners(true)` | Enable the web client to listen to dynamically added DOM elements by enabling [`MutationObserver`](https://developer.mozilla.org/en-US/docs/Web/API/MutationObserver). <br/><br/>When MutationObserver is enabled, event listeners for the target events will be attached to the DOM elements that are added to the DOM after the initial page load. <br/><br/>Enabling MutationObserver does not seem to have a significant impact on application performance, adding approximately 4ms of latency every 1000 target DOM elements. If the number of target DOM elements were to reach a significantly large number, it may have an impact on application performance. <br/><br/>`true`: the web client will enable MutationObserver<br/>`false`: the web client will not enable MutationObserver
