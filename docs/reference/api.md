...
| **setSessionId** | `awsRum.setSessionId('shared-uuid')` | _(NPM only)_ | Adopt an externally-minted session ID. Subsequent dispatches use this ID; already-buffered events inherit it at dispatch time (sessionId rides on `UserDetails` per batch, not per event). Does **not** emit `session_start` and does **not** re-roll sampling. Empty values are ignored with a warning. |
| **startSession** | `awsRum.startSession()` <br> `awsRum.startSession({ sessionId, userId })` | _(NPM only)_ | Begin a new session immediately at a host-known boundary (sign-in, sign-out, kiosk handoff). With no args: mints a fresh UUID, re-rolls sampling, disengages session manual mode, emits `session_start` (subject to `suppressSessionStartEvent`). Optional `sessionId` adopts a host-chosen ID and engages manual mode; optional `userId` rotates the user identity in the same call (silent — no `user_start` event exists). Returns the new session ID. See [Starting a session at runtime](../concepts/sessions.md#starting-a-session-at-runtime). |
...
| **setUserId** | `awsRum.setUserId('shared-uuid')` | _(NPM only)_ | Adopt an externally-supplied user ID. Engages manual mode for the life of the instance, overriding `userIdRetentionDays: 0` and the `allowCookies: false` gate in `getUserId()`. No event is emitted. Empty values are ignored with a warning. |
