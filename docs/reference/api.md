...
| **pinUserId** | `awsRum.pinUserId('shared-uuid')` | _(NPM only)_ | Adopt an externally-supplied user ID. Engages manual mode for the life of the instance, overriding `userIdRetentionDays: 0` and the `allowCookies: false` gate in `getUserId()`. No event is emitted. Empty values are ignored with a warning. |
