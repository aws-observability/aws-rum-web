...
-   `pinSessionId()` and `startSession()` are exposed on both NPM and CDN. `getSessionId()` is NPM-only — the CDN command queue is fire-and-forget and has no return channel, so leaders that need to read their session ID for broadcast must use the NPM build.
...
-   `pinUserId()` is exposed on both NPM and CDN. `getUserId()` is NPM-only — the CDN command queue is fire-and-forget and has no return channel, so leaders that need to read their user ID for broadcast must use the NPM build.
