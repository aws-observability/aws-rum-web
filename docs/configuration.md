...
| sessionId | String | `undefined` | A construct-time session ID seed. When set, the host owns the session lifecycle for this instance — the SDK adopts this ID, never auto-mints a new one, and freezes the session ID across expiry (only the expiry timestamp bumps). Setting `sessionId` also forces `suppressSessionStartEvent: true`, so no `com.amazon.rum.session_start_event` is ever emitted by this instance. To rotate, the host must call `pinSessionId(newId)`. Use this to share a single logical session across multiple instances of the web client (e.g. webview panels, Electron `BrowserWindow`s, or sandboxed micro-frontend iframes) so they appear as one session in CloudWatch RUM. **Session replay is not supported across shared contexts** — only event telemetry (errors, performance, http) is correlated. See [Sharing a session across contexts](./concepts/sessions.md#sharing-a-session-across-contexts). |
