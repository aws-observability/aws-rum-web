...
| applicationAttributes | [MetadataAttributes](#metadataattributes) | `undefined` | Immutable, application-wide metadata that **overrides `sessionAttributes` and auto-collected attributes** (e.g. `domain`, `browserName`) on the request-level `Metadata`. Frozen at construction. Use this to pin values like the deployed `domain`, app name, or build ID that should not change at runtime — `sessionAttributes` covers values that may change during a session. Values must be `string`, `number`, or `boolean`; non-primitive values and keys in the `aws:` reserved namespace are dropped with a warning. |
...
| sessionId | String | `undefined` | A construct-time session ID seed. When set, the host owns the session lifecycle for this instance — the SDK adopts this ID, never auto-mints a new one, and freezes the session ID across expiry (only the expiry timestamp bumps). Setting `sessionId` also forces `suppressSessionStartEvent: true`, so no `com.amazon.rum.session_start_event` is ever emitted by this instance. To rotate, the host must call `pinSessionId(newId)` (or `cwr('pinSessionId', newId)` on the CDN). Use this to share a single logical session across multiple instances of the web client (e.g. webview panels, Electron `BrowserWindow`s, or sandboxed micro-frontend iframes) so they appear as one session in CloudWatch RUM. **Session replay is not supported across shared contexts** — only event telemetry (errors, performance, http) is correlated. See [Sharing a session across contexts](./concepts/sessions.md#sharing-a-session-across-contexts). |
| suppressSessionStartEvent | Boolean | `false` | When `true`, the web client does not emit `com.amazon.rum.session_start_event` on session creation or rotation. Forced to `true` when `sessionId` is seeded at construction (a seeded instance never starts its own session). |
...
| userId | String | `undefined` | A construct-time user ID seed. When set, the host owns the user identity for this instance — the SDK adopts this ID and returns it from `getUserId()` regardless of `userIdRetentionDays` or `allowCookies`. To rotate, the host must call `pinUserId(newId)` (or `cwr('pinUserId', newId)` on the CDN). Use this to pin a single anonymous user identity across multiple instances of the web client (e.g. webview panels, Electron `BrowserWindow`s, or sandboxed micro-frontend iframes) so a single human is not counted as N anonymous users in CloudWatch RUM. See [Sharing a user ID across contexts](./concepts/sessions.md#sharing-a-user-id-across-contexts). |
| userIdRetentionDays | Number | `30` | How long the anonymous user cookie (`cwr_u`) is retained, in days. Set to `0` to disable the user cookie — the web client will use a nil UUID for all users. Overridden by `userId` (manual mode) and `pinUserId()`. |
