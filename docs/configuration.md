...
| stackTraceLength | Number | `1000 ` | The number of characters to record from a JavaScript error's stack trace (if available). |
| recordAllRequests | boolean | `true` | By default, all HTTP requests are recorded. When this field is `false`, http telemetry will only record failed requests (i.e., those with network errors or status codes which are not 2xx). <br/><br/>This field does **does not apply** to X-Ray traces, where all requests are recorded. |
