...
| applicationAttributes | [MetadataAttributes](#metadataattributes) | `undefined` | Immutable, application-wide metadata that **overrides `sessionAttributes` and auto-collected attributes** (e.g. `domain`, `browserName`) on the request-level `Metadata`. Frozen at construction. Use this to pin values like the deployed `domain`, app name, or build ID that should not change at runtime — `sessionAttributes` covers values that may change during a session. The `aws:` namespace is reserved and dropped with a warning. |
