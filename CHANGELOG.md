...
* **cd:** harden release workflow ([5ee32d9](https://github.com/aws-observability/aws-rum-web/commit/5ee32d94244ea1495d7ad009103def0188abd276))
...
All notable changes to this project will be documented in this file. See [Conventional Commits](https://www.conventionalcommits.org/) for commit guidelines. Starting with v3.0.0 this changelog is generated by [lerna version](https://lerna.js.org/docs/features/version-and-publish).

## [3.0.0](https://github.com/aws-observability/aws-rum-web/compare/v2.1.0...v3.0.0) (2026-05-01)

> **Note:** v3.0.0 is a one-time, hand-written entry covering the 2.1.0 → 3.0.0 monorepo cutover. Subsequent releases are generated by `lerna version --conventional-commits`.

### ⚠ BREAKING CHANGES

* **monorepo:** repository restructured into a monorepo. Three packages are now published: `@aws-rum-web/core` (shared runtime), `@aws-rum-web/slim` (BYO-auth entry point), and `aws-rum-web` (full client, now extends slim). Existing `aws-rum-web` consumers see no import changes.
* **build:** compilation target bumped from ES5 to ES2017. Browsers without native `async`/`await`, classes, or object spread are no longer supported.
* **config:** `telemetries` and auth-related fields moved from core `Config` to web `Config`. Apps importing those types from the core package must update imports.
* **telemetries:** default telemetries changed to `errors`, `performance`, and `http`.
* **compression:** gzip compression is enabled by default.
* **session replay:** `rrweb` session replay is enabled by default in the web package.
* **slim:** replaces `ua-parser-js` with the native `userAgentData` API. Environments without `navigator.userAgentData` fall back to a minimal parser and may report less detail.
* **ci/cd:** build pipeline upgraded to Node.js 20.

### Features

* **slim:** new `@aws-rum-web/slim` package with BYO-auth contract and `clearCookies()` helper ([715f61f](../../commit/715f61f))
* **web:** `aws-rum-web` now extends slim as a shared base, reducing duplication ([cc69447](../../commit/cc69447))
* **rrweb:** add session replay plugin with flush lifecycle hook and privacy masking enforcement ([e5c831c](../../commit/e5c831c), [4f76dc6](../../commit/4f76dc6)); enable session replay by default ([3271b9e](../../commit/3271b9e))
* **core:** add `WEB_CLIENT_VERSION` utility and version test ([3fd4f79](../../commit/3fd4f79))
* **compression:** enable compression by default ([3fe7010](../../commit/3fe7010))
* **telemetries:** default to `errors`, `performance`, `http` ([ee949aa](../../commit/ee949aa))
* **examples:** add slim/full toggle harness for local validation ([849ef9b](../../commit/849ef9b)); add plugin loading and session replay to `spa-react-demo` ([ff83320](../../commit/ff83320))

### Bug Fixes

* **build:** add NPM dist builds for `aws-rum-web` and `aws-rum-slim` ([7437178](../../commit/7437178))
* **ci:** parallelize and consolidate smoke tests; fix integ and smoke tests for monorepo ([cc04d6c](../../commit/cc04d6c), [582ca4b](../../commit/582ca4b), [cfc8260](../../commit/cfc8260))
* **dev:** resolve security violations ([de24791](../../commit/de24791))
* exports: remove rrweb plugin from top-level exports ([87ed51d](../../commit/87ed51d))

### Refactors

* restructure into monorepo with `core`, `aws-rum-web`, and `slim` packages ([9518015](../../commit/9518015))
* **signing:** move `createSigningConfig` from web to core ([672e5ea](../../commit/672e5ea))
* **config:** move telemetries and auth fields from core to web ([27f8a6d](../../commit/27f8a6d), [2eaa28b](../../commit/2eaa28b))
* migrate from deep to barrel imports ([c632fc6](../../commit/c632fc6))
* consolidate tests and rename packages ([5d53992](../../commit/5d53992))

### Chores

* migrate to `@smithy/querystring-builder` ([d70a72d](../../commit/d70a72d))
* remove unused `@babel/runtime` dependency ([9424a3b](../../commit/9424a3b))
* add check for package size regression ([cd8a985](../../commit/cd8a985))
* upgrade CD to Node.js 20 ([82dcdf9](../../commit/82dcdf9))
* **ci/cd:** publish all monorepo packages to NPM; split publish into separate retryable steps ([c391038](../../commit/c391038), [425f0ed](../../commit/425f0ed))

### Documentation

* restructure documentation: unified Getting Started, slim reference, full→slim migration guide, public-policy auth flow, size callout ([43206f7](../../commit/43206f7), [8b33242](../../commit/8b33242))
* update CDN URLs to 3.x and mark RRWebPlugin as GA
* rebuild `mpa-cdn-demo` and `spa-react-demo` examples for 1:1 parity
