...
CDN (embedded script): paste the snippet from the RUM console into your `<head>`. The snippet loads the bundle from the AWS-hosted CDN — see [CDN bundle URLs](#cdn-bundle-urls) below, or [CDN installation](docs/cdn_installation.md) for the full snippet reference.
...
The web client is published to an AWS-hosted CDN. The snippet generated by the RUM console already uses the recommended URL, but you can also reference these directly:
