 ...
 */
export class EventCache extends Configurable<Config> {
    private readonly appMonitorDetails: AppMonitorDetails;
    ...
    public getEventBatch(): RumEvent[] {
        ...
        if (this.events.length <= this.getConfigValue('batchLimit')) {
            // Return all events.
            rumEvents = this.events;
            this.events = [];
        } else {
            // Dispatch the front of the array and retain the back of the array.
            rumEvents = this.events.splice(
                0,
                this.getConfigValue('batchLimit')
            );
        }
    ...
    private canRecord = (session): boolean => {
        return (
            session.record &&
            (session.eventCount <= this.getConfigValue('sessionEventLimit') ||
                this.getConfigValue('sessionEventLimit') <= 0)
        );
    };
    ...
    private addRecordToCache = (type: string, eventData: object) => {
        ...
        if (this.events.length === this.getConfigValue('eventCacheSize')) {
    ...
    private isCurrentUrlAllowed() {
        ...
        if (
            this.getConfigValue('pagesToExclude')?.length > 0 &&
            this.getConfigValue('pagesToExclude')?.some((re) =>
                re.test(location)
            )
        ) {
            return false;
        ...
        if (
            !this.getConfigValue('pagesToInclude')?.length ||
            this.getConfigValue('pagesToInclude')?.some((re) =>
                re.test(location)
            )
        ) {
            return true;
