...
export class EventCache {
    ...
    public getValidMetadata(metadata: object): MetaData {
        let count = 0;
        const validMetadata = {};

        // include all default attributes
        ...
        Object.keys(metadata).forEach((attributeKey) => {
            let isValidAttribute = false;
            const attributeValue = metadata[attributeKey];

            if (attributeKey === 'pageTags' && Array.isArray(attributeValue)) {
                isValidAttribute = true;
            } else {
                const isValidType = VALID_ATTRIBUTE_TYPES.includes(
                    typeof attributeValue
                );
                const isValidAttributeKey = ATTRIBUTE_KEY_REGEX.test(
                    attributeKey
                );
                const isValidAttributeValueLength =
                    String(attributeValue).length <= MAX_ATTRIBUTE_KEY_LENGTH;
                isValidAttribute =
                    isValidType &&
                    isValidAttributeKey &&
                    isValidAttributeValueLength;
            }
            // only include up to allowed limit of custom attributes
            if (count < RUM_CUSTOM_ATTRIBUTE_LIMIT_VALUE) {
                if (isValidAttribute) {
                    validMetadata[attributeKey] = metadata[attributeKey];
                    count++;
                }
            } else {
    ...
    private addRecordToCache = (type: string, eventData: object) => {
        ...
        const validMetaData = this.getValidMetadata(metaData);

        this.events.push({
            details: JSON.stringify(eventData),
            id: v4(),
            metadata: JSON.stringify(validMetaData),
            timestamp: new Date(),
            type
        });
    };
