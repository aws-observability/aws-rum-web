...
fixture('Custom Events Plugin').page('http://localhost:8080/custom_event.html');

const removeUnwantedEvents = (json: any) => {
    const newEventsList = json.RumEvents.filter((e) =>
...
test('when a plugin calls recordEvent x times then event is recorded x times', async (t: TestController) => {
    // This test succeeds when run on its own, but is flaky on Microsoft Edge when run as apart of the suite.
...
test('when plugin recordEvent has empty event_data then RumEvent details is empty', async (t: TestController) => {
    // This test succeeds when run on its own
    if (t.browser.name.includes('Edge')) {
        return 'Test is skipped';
    }
